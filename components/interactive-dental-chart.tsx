...
export function InteractiveDentalChart({ onToothSelect }: InteractiveDentalChartProps) {
  ...
  const [contextMenu, setContextMenu] = useState<{
    ...
    y: number
    toothNumber: string
  }>({ show: false, x: 0, y: 0, toothNumber: "" })

  // FDI tooth numbering system - Adult teeth
  const upperTeeth = ["18", "17", "16", "15", "14", "13", "12", "11", "21", "22", "23", "24", "25", "26", "27", "28"]
  const lowerTeeth = ["48", "47", "46", "45", "44", "43", "42", "41", "31", "32", "33", "34", "35", "36", "37", "38"]

  // Mock tooth data with various conditions - using state to allow updates
  ...
  const handleToothRightClick = (e: React.MouseEvent, toothNumber: string) => {
    e.preventDefault()
    setContextMenu({
      show: true,
      x: e.clientX,
  ...
  const handleContextMenuClose = () => {
    setContextMenu({ show: false, x: 0, y: 0, toothNumber: "" })
  }

  const handleQuickStatusChange = (toothNumber: string, status: "healthy" | "caries" | "filled" | "crown" | "missing" | "attention") => {
    setToothData(prev => ({
      ...prev,
      [toothNumber]: {
        number: toothNumber,
        status,
        ...(status === "missing" ? {} : { diagnosis: getDefaultDiagnosis(status) })
      }
    }))
    handleContextMenuClose()
  }

  const getDefaultDiagnosis = (status: string) => {
    switch (status) {
      case "caries": return "Requires treatment"
      case "filled": return "Restoration present"
      case "crown": return "Crown present"
      case "attention": return "Requires evaluation"
  ...
  const renderTooth = (toothNumber: string, isUpper = true) => {
    ...
    return (
      <div
        ...
        onContextMenu={(e) => handleToothRightClick(e, toothNumber)}
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-4 gap-4">
        ...
        <Card>
          <CardContent className="p-4 text-center">
            <div className="text-2xl font-bold text-red-600">{stats.caries}</div>
            <div className="text-xs text-muted-foreground">Caries</div>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="p-4 text-center">
            <div className="text-2xl font-bold text-blue-600">{stats.restorations}</div>
          ...
          >
            ...
            <div className="space-y-1">
              ...
              <button
                ...
                onClick={() => handleQuickStatusChange(contextMenu.toothNumber, "caries")}
              >
              ...
              <button
                ...
                onClick={() => handleQuickStatusChange(contextMenu.toothNumber, "filled")}
              >
              ...
              <button
                ...
                onClick={() => handleQuickStatusChange(contextMenu.toothNumber, "crown")}
              >
              ...
              <button
                ...
                onClick={() => handleQuickStatusChange(contextMenu.toothNumber, "missing")}
              >
                <div className="w-3 h-3 bg-gray-200 border border-gray-400 rounded"></div>
                Missing
              </button>
              <button
                className="w-full text-left px-3 py-2 text-sm hover:bg-orange-50 rounded flex items-center gap-2"
                onClick={() => handleQuickStatusChange(contextMenu.toothNumber, "attention")}
              >
      ...
      {selectedTooth && (
        <ToothDiagnosisDialog
          ...
          onSave={(toothNumber, diagnoses, treatments) => {
            const primaryDiagnosis = diagnoses[0]
            let status: "healthy" | "caries" | "filled" | "crown" | "missing" | "attention" = "healthy"
            
            if (primaryDiagnosis) {
              if (primaryDiagnosis.toLowerCase().includes("caries")) {
                status = "caries"
              } else if (primaryDiagnosis.toLowerCase().includes("restoration") || primaryDiagnosis.toLowerCase().includes("filling")) {
                status = "filled"
              } else if (primaryDiagnosis.toLowerCase().includes("crown")) {
                status = "crown"
              } else if (primaryDiagnosis.toLowerCase().includes("missing")) {
                status = "missing"
              } else {
                status = "attention"
              }
